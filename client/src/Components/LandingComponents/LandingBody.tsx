...
export default function LandingBody() {
  ...
  const { dispatch, currentUser, darkTheme } = useGeneralAppContext()
  ...
  return (
    <div className='pb-4 min-h-[85vh] flex items-center justify-center flex-col mx-10 max-w-[600px] lg:max-w-[800px] landing-bg relative'>
      <div className={`${darkTheme ? 'text-[#f4f5fd] bg-[#ffebcd1a]' : 'text-[#192657] bg-[#1926571a]'} z-[99999] flex  gap-2 items-center rounded-full py-[10px] px-[12px] `}>
        <i className='text-[1.2rem]'><RiCompassFill /></i>
        <p className='text-[0.9rem] '>The easiest way to build a resume</p>
      </div>
      <h1 className={`z-[99999] font-bold text-[2rem] lg:text-[3rem] text-center ${darkTheme ? 'text-[#ffebcd]':'text-[#192657]'}  mt-7`}>Build Your Professional Resume with our Creative Templates</h1>
      <p className={`z-[99999] text-center ${darkTheme ? 'text-[#f9f9f9]':'text-[#333333]'} text-[0.8rem] lg:text-base mt-8 leading-6 lg:leading-8`}>
