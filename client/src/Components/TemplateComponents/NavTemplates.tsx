...
export default function NavTemplates() {
  ...
  return (
    <div className="bg-[#ffffff] w-full z-[999999] sticky top-0 landingShadow py-[1rem] px-8 md:px-[3rem] lg:px-[6rem] flex justify-between items-center">
      ...
      <button onClick={navigateTo}  className={`${state?.isSelected ? ' gradientForChooseTemplate' : ' border-[#747C99] text-[#747C99]'} hidden md:block rounded-md py-3 px-7 border-[1px] font-medium duration-300 ease-in transition-colors`}>Choose Template</button>
    </div>  
  )
}
